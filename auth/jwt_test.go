...
import (
	...
	"github.com/afushimi-source/go_todo_app/clock"
	"github.com/afushimi-source/go_todo_app/entity"
	"github.com/afushimi-source/go_todo_app/store"
	"github.com/afushimi-source/go_todo_app/testutil/fixture"
...
func TestJwter_GenarateToken(t *testing.T) {
...
func TestJWTer_GetToken(t *testing.T) {
	...
	want, err := jwt.NewBuilder().
		...
		Issuer(`github.com/afuhimi-source/go_todo_app`).
	...
	req := httptest.NewRequest(
		http.MethodGet,
		`https:github.com/afushimi-source`,
		nil,
	)
...
func TestJWTer_GetToken_NG(t *testing.T) {
	...
	tok, err := jwt.NewBuilder().
		JwtID(uuid.New().String()).
		Issuer("github.com/afushimi-source/go_todo_app").
		Subject("access_token").
		IssuedAt(c.Now()).
		Expiration(c.Now().Add(30*time.Minute)).
		Claim(RoleKey, "test").
